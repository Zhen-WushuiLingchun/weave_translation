...
function Options(): React.ReactElement {
  ...
  useEffect(() => { void sendRuntimeMessage<WeaveSettings>({ type:'GET_SETTINGS' }).then((loaded) => { setSettings(loaded); setDraft(loaded.provider); }); }, []);
  ...
  return <main className="settings-layout">
    ...
    <section className="settings-content">
      ...
      {section==='provider' && <div className="settings-card provider-grid">
        <label className="field"><span>服务类型</span><select value={draft.kind} onChange={(e)=>setDraft({...draft,kind:e.target.value as ProviderProfile['kind'],...(e.target.value==='deepseek'?{label:'DeepSeek',endpoint:'https://api.deepseek.com/chat/completions',model:'deepseek-v4-flash'}:{label:'自定义接口'})})}><option value="deepseek">DeepSeek</option><option value="openai-compatible">OpenAI-compatible</option></select></label>
        ...
        <label className="field full"><span>模型思考强度</span><select value={draft.reasoningMode} onChange={(e)=>setDraft({...draft,reasoningMode:e.target.value as ProviderProfile['reasoningMode']})}><option value="compatible">兼容模式 · 不发送推理参数</option><option value="fast">快速 · 最低延迟</option><option value="balanced">均衡 · 兼顾速度与语境</option><option value="deep">深入 · 复杂长文优先</option></select><small>{draft.kind==='deepseek'?'DeepSeek：快速关闭 thinking；均衡为 high；深入为 max。':'OpenAI-compatible：依次发送 none、medium、high；不支持时请选择兼容模式。'}</small></label>
