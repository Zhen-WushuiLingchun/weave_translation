...
def multipart(audio: Path, model: str) -> tuple[bytes, str]:
    boundary = f"----weave-{uuid.uuid4().hex}"
    chunks: list[bytes] = []
    for name, value in (("model", model), ("language", "en"), ("response_format", "verbose_json")):
...
payload, boundary = multipart(args.audio, args.model)
