...
无字幕视频需要额外配置完整的 `/audio/transcriptions` 地址和具备 `audioTranscription` 能力的模型。仓库提供可选的 Windows 本地服务安装脚本，支持 NVIDIA CUDA、Intel 核显和 CPU；详见 [本地 ASR 配置](docs/local-asr.md)。
