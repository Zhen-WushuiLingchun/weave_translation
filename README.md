...
- 所有普通网页默认可见、左右吸附并自动收回的侧边坞；关闭时只有可见把手响应点击
- 原文 / 双语 / 译文三种整页模式，视口优先、支持动态页面
- 带段落和标题上下文的划词翻译及语境解释；点击后立即弹出分阶段加载卡片，卡片可拖动并受视口边界约束
- YouTube、Bilibili 现有字幕读取、语义重组、双语覆盖与提前翻译
- DeepSeek 预设、任意 Chat Completions 兼容地址、本地模型地址
- 网页 / 划词 / 视频分别设置兼容、快速、均衡或深入四档推理控制，并按 DeepSeek 或 OpenAI 格式转换参数
- 站点翻译档案：`example.com` 覆盖该主机的全部路径，`*.example.com` 覆盖主域名和所有层级子域名；可覆盖自动翻译、页面模式、目标语言、推理强度和主题
- 自动识别网页浅色 / 深色背景，也可在默认设置或站点档案中强制译文界面主题
- 网站切换主题时即使重建 `body`，侧边坞也会保留并重新绑定主题监听
- 侧边把手使用 Chrome 网页原生顶层栈与最高层级保护，普通遮罩、弹层及常见容器全屏不会盖住它
...
1. 安装后，织语会在普通 `http/https` 网页自动显示侧边把手；点击把手展开，不会悬停误触。
2. 打开设置，在“模型服务”中选择 DeepSeek 或填写完整 OpenAI-compatible Chat Completions 地址。
3. 填写 API Key、模型名并执行“保存并测试”。本地服务可以留空 Key；旧模型不接受推理参数时，在网页、划词或视频页面选择“兼容模式”。
4. 在“网页翻译”中设置默认模式和站点翻译档案；也可在任意网页右侧展开“织语”把手，为当前主机快速保存覆盖设置。
5. 在 YouTube/Bilibili 使用已有字幕的视频中开启字幕翻译；YouTube 若未发现字幕轨，先开启一次原生 CC。

## 安全边界

- 安装时申请普通 `http/https` 网页访问权限，用于默认显示侧边栏；Chrome 内部页与 Chrome Web Store 仍无法注入。
...
- 显示侧边栏本身不会读取正文或调用模型；只有用户主动翻译或站点自动规则生效时，网页文本才会发送到所配置的模型地址。
